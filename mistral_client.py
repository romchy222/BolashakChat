...
class MistralClient:
    ...
    def _get_smart_fallback_response(self,
                                     ...
                                     context: str,
                                     language: str = "ru") -> str:
        """Get smart fallback response based on available context from KB"""
        
        # If we have context from KB, use it
        if context and context.strip():
            if language == "kz":
                return (f"**Білім базасынан ақпарат:**\n\n{context[:500]}...\n\n"
                        "Толық ақпарат алу үшін университет әкімшілігіне хабарласыңыз.")
            return (
                f"**Информация из базы знаний:**\n\n{context[:500]}...\n\n"
                "Для получения полной информации обратитесь в администрацию университета."
            )

        # If no context available, provide basic fallback
        return self._get_fallback_response(language)
